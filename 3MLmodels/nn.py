...
class SquareLoss(FunctionNode):
    ...
    def _forward(*inputs):
        ...
        assert inputs[0].shape == inputs[1].shape, (
            "Input shapes should match, instead got {} and {}".format(
                format_shape(inputs[0].shape), format_shape(inputs[1].shape)))
        #print(np.mean(np.square(inputs[0] - inputs[1]) / 2))
    ...
    def _backward(gradient, *inputs):
        ...
        "*** YOUR CODE HERE (Q4) ***"
        result1 = (inputs[0] - inputs[1]) * gradient / inputs[0].size
        result2 = -result1
        #print(inputs[0])
        #print(gradient)
...
class SoftmaxLoss(FunctionNode):
    ...
    def _backward(gradient, *inputs):
        ...
        "*** YOUR CODE HERE (Q5) ***"
        result1 = gradient * (np.exp(log_probs) - inputs[1]) / inputs[0].shape[0]
        result2 = gradient * (-1) * log_probs / inputs[0].shape[0]
        return [result1, result2]
